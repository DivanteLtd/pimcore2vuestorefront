...
class Message {
  ...
  static greeting(text, isLastMessage = false) {
    ...
    message([...text], Object.assign(isLastMessage ? { marginTop: 1 } : {}, { borderColor: 'green', marginBottom: 1 }));
  }
}
...
class Manager extends Abstract {
   ...
   */
  initPimcore() {
    return this.storefront.goToDirectory().then(this.storefront.createConfig.bind(this.storefront)).then(this.storefront.npmBuild.bind(this.storefront)).then(this.storefront.runDevEnvironment.bind(this.storefront));
  }
  ...
  static checkUserOS() {
    if (isWindows()) {
      Message.error(['Unfortunately currently only Linux and OSX are supported.', 'To install vue-storefront on your mac please go threw manual installation process provided in documentation:', `${STOREFRONT_GIT_URL}/blob/master/doc/Installing%20on%20Windows.md`]);
    }
  }
  ...
  static showWelcomeMessage() {
    Message.greeting(['Hi, welcome to the vue-storefront installation.', 'Let\'s configure it together :)']);
  }
...
if (require.main.filename === __filename) {
  ...
  inquirer.prompt(questions).then(async function (answers) {
    ...
    await manager.tryToCreateLogFiles().then(manager.initBackend.bind(manager)).then(manager.initStorefront.bind(manager)).then(manager.showGoodbyeMessage.bind(manager)).catch(Message.error);
