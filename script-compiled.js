...
class Manager extends Abstract {
  ...
  tryToCreateLogFiles() {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        let logFiles = [INSTALL_LOG_FILE, GENERAL_LOG_FILE];
   ...
   */
  initPimcore() {
    return this.pimcore.createConfig.bind(this.pimcore)().then(this.pimcore.runImporter.bind(this.pimcore));
  }
  ...
  static showWelcomeMessage() {
    Message.greeting(['Hi, welcome to the pimcore2vuestorefront setup.', 'Let\'s configure it together :)']);
  }
  ...
  showGoodbyeMessage() {
    return new Promise((resolve, reject) => {
      Message.greeting(['Congratulations!', '', 'You\'ve just configured Pimcore -> VueStorefront integrator.', '', 'Good Luck!'], true);
...
let questions = [{
    ...
    try {
      api.get('classes').end(resp => {
        if (resp.body.success == false) {
          ...
          availablePimcoreClassess = resp.body.data.map(itm => {
            return itm.name;
          });
          done(null, true);
        }
  ...
  choices: function (answers) {
    return availablePimcoreClassess;
  },
...
if (require.main.filename === __filename) {
  ...
  inquirer.prompt(questions).then(async function (answers) {
    ...
    await manager.tryToCreateLogFiles().then(manager.initPimcore.bind(manager)).then(manager.showGoodbyeMessage.bind(manager)).catch(Message.error);
